...
def init_dataset_kwargs(data, imgsize):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data, use_labels=True, max_size=None, xflip=False)
        dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
        #dataset_kwargs.resolution = dataset_obj.resolution # Be explicit about resolution.
        dataset_kwargs.resolution = imgsize
...
# Required.
@click.option('--volume',       help='volume directory',        metavar='DIR', default='moai',              required=True)
@click.option('--project',      help='project directory',       metavar='DIR', default='test_project',      required=True)
@click.option('--subproject',   help='subproject directory',    metavar='DIR', default='test_subproject',   required=True)
@click.option('--task',         help='task directory',          metavar='DIR', default='test_task',         required=True)
@click.option('--version',      help='version',                 metavar='DIR', default='v1',                required=True)

@click.option('--outdir',       help='Where to save the results',   metavar='DIR')
@click.option('--cfg',          help='Base configuration',                                  type=click.Choice(['stylegan3-t', 'stylegan3-r', 'stylegan2', 'fastgan']))
...
@click.option('--batch_gpu',    help='Limit batch size per GPU', metavar='INT',                 type=click.IntRange(min=1))
@click.option('--cbase',        help='Capacity multiplier', metavar='INT',                      type=click.IntRange(min=1), default=32768, show_default=True)
@click.option('--cmax',         help='Max. feature maps', metavar='INT',                        type=click.IntRange(min=1), default=512, show_default=True)
@click.option('--glr',          help='G learning rate  [default: varies]', metavar='FLOAT',     type=click.FloatRange(min=0))
@click.option('--dlr',          help='D learning rate', metavar='FLOAT',                        type=click.FloatRange(min=0), default=0.002, show_default=True)
@click.option('--map_depth',    help='Mapping network depth  [default: varies]', metavar='INT', type=click.IntRange(min=1))
...
def main(**kwargs):
    ...
    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, imgsize=opts.imgsize)
